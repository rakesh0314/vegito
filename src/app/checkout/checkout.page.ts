...
export class CheckoutPage implements OnInit {
  ...
  validinput()
  {
    ...
    return(this.order.pay_op&&payment&&this.address!='');
  }
  ...
  placeOrder()
  {
    if(this.validinput())
    {
      this.storageService.getdata("vegisession").then(res=>
        {
          this.apiService.makeOrder(res,this.order.pay_op,this.product,this.address).subscribe((result)=>
          {
            if(result.status==true && result.gateway=="open")
            {
              this.placeOrders(result.order_id);
            }else{
              this.storageService.getdata('vegitoOrder').then(resl=>
                {
                  if(resl==null)
                  {
                    this.storageService.setdata('vegitoOrder',result).then(()=>
                    {
                    ...
                    let array = [resl,result];
                    this.storageService.setdata('vegitoOrder',array).then(()=>
                    {
                      let msg = 'Order Successfully';
                      this.apiService.showmsg(msg);
                      this.router.navigate(['payment-confirmation']);
          ...
          {
            this.apiService.showmsg('error while create order please try again later..');
          });
        });
    }else{
  ...
  placeOrders(order_id)
  {
    ...
    var successCallback = (payment_id) => {
      ...
      let array ={
        orderId:order_id,
        price:this.totalAmountsell,
        txnId:payment_id,
        userId:this.userData.id
      }
      this.apiService.updateOrder(array).subscribe((res)=>
      {
        this.storageService.getdata('vegitoOrder').then(resl=>
        {
          if(resl==null)
          {
            this.storageService.setdata('vegitoOrder',res).then(()=>
            {
              let msg = 'Order Successfully';
              this.apiService.showmsg(msg);
              this.router.navigate(['payment-confirmation']);
